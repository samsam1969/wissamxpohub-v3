...
async def get_export_advice(
    ...
    # ── Layer 0: Knowledge Base (Internal - Highest Priority)
    kb_data = ""
    try:
...
"""
    ...
    # Merge Tavily sources with existing sources
    if tavily_sources:
        for ts in tavily_sources:
            if ts["url"] not in [s if isinstance(s, str) else s.get("url","") for s in source_urls]:
                source_urls.append({"name": ts.get("name",""), "url": ts.get("url",""), "type": "tavily"})
